...
class UpbitAPI:
    ...
    def _get_authorization_token(self, query=None):
        ...
        jwt_token = jwt.encode(payload, self.secret_key, algorithm='HS256') # HS256은 SHA256 알고리즘. SECTRET_KEY를 디지털 싸인으로 변환시켜 사용자가 보낸 요청인지 여부를 확인하는 데 사용하도록 함. SECRETE_KEY의 진위를 확인 즉, 사용자가 본인인지를 확인 할 수 있는 기능
